...
class ExampleList: ListBaseCD {
    ...
    override func viewDidLoad() {
        ...
        frc = m.ds_frcForEntityNamed("Person", withPredicateFormat: nil, predicateObject: nil, sortDescriptors: [NSSortDescriptor(key: "province", ascending: true, selector: #selector(NSString.caseInsensitiveCompare(_:))), NSSortDescriptor(key: "city", ascending: true)], andSectionNameKeyPath: "province")
        ...
        // Alternative, create the frc WITH a predicate
        // Show only items that begin with "o" and their quantity is less than 125
        /*
         let limit = 125
         let letter = "o"
         frc = m.ds_frcForEntityNamed("Example", withPredicateFormat: "quantity < %@ && name BEGINSWITH[cd] %@", predicateObject: [limit as AnyObject, letter as AnyObject], sortDescriptors: [NSSortDescriptor(key: "name", ascending: true, selector: #selector(NSString.caseInsensitiveCompare(_:)))], andSectionNameKeyPath: nil)
         */
    ...
    override func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        ...
        let s = self.frc.sections?[section]
        // Use it as the section title
        return "Persons in \(s?.name ?? "none")"
    }
