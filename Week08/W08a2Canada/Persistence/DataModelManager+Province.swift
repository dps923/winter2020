//
//  DataModelManager+Province.swift
...
extension DataModelManager {
    ...
    func province_Count(withPredicate: NSPredicate) -> Int {
    ...
    // Fetch all
    func province_GetAll() -> [Province]? {
        return province_GetSome(withPredicate: nil)
    }
    
    // Fetch some
    func province_GetSome(withPredicate: NSPredicate?) -> [Province]? {
    ...
    func province_GetByObjectId(_ objectId: NSManagedObjectID) -> Province? {
    ...
    func province_GetByName(_ name: String) -> Province? {
    ...
    // Create new
    func province_CreateItem() -> Province? {
        
        guard let newItem = NSEntityDescription.insertNewObject(forEntityName: "Province", into: ds_context) as? Province else {
    ...
    // Delete item
    func province_DeleteItem(item: Province) {
        ds_context.delete(item)
        ds_save()
    }
    
    // Delete all
    func province_DeleteAll() {
        if let result = province_GetAll() {
            for obj in result {
