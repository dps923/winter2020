...
class DataModelManager {
    ...
    func postAddNew(_ postData: TCPost) {
        
        // Create a request object (and configure it if necessary)
        let request = WebApiRequest()
        
        // Configure the request
        request.urlBase = "https://jsonplaceholder.typicode.com"
        ...
        // Send the request, and write a completion method to pass to the request
        request.sendRequest(toUrlPath: "/posts") { (result: TCPost) in
        ...
        // Post a notification
        NotificationCenter.default.post(name: Notification.Name("TypicodePostWasSuccessful"), object: nil)
    }
