...
class ProvinceList: ListBaseCD, AddProvinceDelegate, ShowProvinceDetailDelegate {
    
    // MARK: - Private internal instance variables
    
    // Configure the desired entity type in the frc
    private var frc: NSFetchedResultsController<Province>!
    ...
    override func viewDidLoad() {
        ...
        frc = m.ds_frcForEntityNamed("Province", withPredicateFormat: nil, predicateObject: nil, sortDescriptors: [NSSortDescriptor(key: "name", ascending: true, selector: #selector(NSString.caseInsensitiveCompare(_:)))], andSectionNameKeyPath: nil)
        
        // Alternative, case-sensitive
        //frc = m.ds_frcForEntityNamed("Province", withPredicateFormat: nil, predicateObject: nil, sortDescriptors: [NSSortDescriptor(key: "name", ascending: true)], andSectionNameKeyPath: nil)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.detailTextLabel?.text = "Premier is \(item.premier!)"
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if editingStyle == .delete {
            ...
            let item = frc.object(at: indexPath)
            m.province_DeleteItem(item: item)
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        /*
         ...
         // so we must go through it
         let nav = segue.destination as! UINavigationController
         let vc = nav.viewControllers[0] as! ProvinceDetailCD
