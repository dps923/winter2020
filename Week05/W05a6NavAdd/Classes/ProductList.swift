...
class ProductList: UITableViewController, ShowProductDetailDelegate, AddProductDelegate {
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            let item = m.productPackage.data[indexPath.row]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "toProductScene1" {
        ...
        if segue.identifier == "toProductAdd" {
            
            // Get a reference to the controller
            let nav = segue.destination as! UINavigationController
            let vc = nav.viewControllers[0] as! ProductAdd
